...
get_day_point <- function(df, day_column){
  if(class(df[[day_column]])=="Date"){
    print("Variable is `Date` format, continue.")
  }else{
      stop("Not a date format, please use 'lubridate' package.")
  }
  df %>%
    mutate_at(day_column, list(date_year=year, date_month=month, date_day=day)) 
}
...
get_day_point(USvideos_NNA, "trending_date")%>%
  ...
  assert(in_set(seq(1, 12, 1), allow.na = FALSE), date_month) %>% 
  assert(in_set(seq(1, 31, 1), allow.na = FALSE), date_day) %>% 
  assert(in_set(2017, 2018, allow.na = FALSE), date_year)

```
# There was no abnormal data
...
get_time_point(USvideos_NNA, "publish_time") %>%
  ...
  assert(in_set(seq(2006,2018,1), allow.na = FALSE), time_year) %>% 
  assert(in_set(seq(1,12,1), allow.na = FALSE), time_month) %>% 
  assert(in_set(seq(1,31,1), allow.na = FALSE), time_day) %>% 
  assert(in_set(seq(0,23,1), allow.na = FALSE), time_hour) %>% 
  assert(in_set(seq(0,59,1), allow.na = FALSE), time_minute) %>% 
  assert(in_set(seq(0,59,1), allow.na = FALSE), time_second)
```
...
get_weekday(USvideos_NNA, publish_time) %>%
  ...
  assert(in_set(c("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"), allow.na = FALSE), 
         weekday)
```
...
count_pattern(USvideos_NNA, tags) %>%
  ...
  assert(within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), pattern_num)
```
...
count_uppercase(USvideos_NNA, title) %>%
  ...
  assert(within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), chr_upper_num)
...
count_symbol(USvideos_NNA, title) %>%
  ...
  assert(within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), chr_symbol_num)
```
...
get_chr_length(USvideos_NNA, title) %>%
  ...
  assert(within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), chr_length)
...
get_ratio(USvideos_NNA, likes, dislikes) %>%
  ...
  assert(within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), likes_rate)
...
convert_to_01(USvideos_NNA, comments_disabled, ratings_disabled, video_error_or_removed) %>%
  ...
  assert(in_set(1, 0, allow.na = FALSE), comments_disabled) %>% 
  assert(in_set(1, 0, allow.na = FALSE), ratings_disabled) %>% 
  assert(in_set(1, 0, allow.na = FALSE), video_error_or_removed) 
```
...
class_level(USvideos_NNA, views) %>%
  select(views,level) %>% 
  assert(in_set("level1", "level2","level3","level4", allow.na = FALSE), level)
