...
Create a function for splitting the trending date column into 3 columns. And in this part, we can verify the column type first to make sure it's date type or not. If it isn't a date type column, the function will stop and reture "Not a date format, please use 'lubridate' package."
...
Definitely, the weekday that youtubers post the video will influence the views of videos in the first several publish day. According to Matthew effect, the video with more views in the very begining will have more possibility to be trending.
```{r get_weekday}
...
Now we will create a function to count the number of rows in decription column. 
The rows' number of description represents the length of the description. A description of the appropriate length may help viewers understand the video better, and make the video be accepted by more people.
The pattern of Line break symbol is "\\n". Count the number of "\\n" and plus 1, the result should be the the number of row.
```{r count_rows}
...
This function is similar to the last one but it is used to count the number of tags. Youtubers use tags to help viewers find the video conveniently. As a result, suitable number of tags will be a factor what make a video trending.

This dataset use "|" to split the tags. So the number of tags should be the number of "|" plus 1.
```{r count_pattern}
...
Sometimes we might need calculate the ratio between 2 variables. This function can help us to get the ratio column.For example, the ratio between likes and dislikes will show us the controversy about the video. Hint: denominator + 0.001 to make sure there is no Nah when denominator is 0.
```{r get_ratio}
get_ratio <- function(df, numerator, denominator){
  
  df %>%
    mutate(likes_rate = {{numerator}}/({{denominator}}+0.001))
}
```
...
## Test the feature engineering results

```{r test the FE results, eval=FALSE}
assert(data = USvideos_NNA, in_set(1, 0, allow.na = FALSE), comments_disabled)
assert(data = USvideos_NNA, in_set(1, 0, allow.na = FALSE), ratings_disabled)
assert(data = USvideos_NNA, in_set(1, 0, allow.na = FALSE), video_error_or_removed) 
assert(data = USvideos_NNA, in_set("level1", "level2","level3","level4", allow.na = FALSE), level)
assert(data = USvideos_NNA, within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), likes_rate)
# assert(data = USvideos_NNA, in_set(2017, 2018, allow.na = FALSE), date_year)
# assert(data = USvideos_NNA, in_set(seq(1,12,1), allow.na = FALSE), date_month)
# assert(data = USvideos_NNA, in_set(seq(1,31,1), allow.na = FALSE), date_day)
assert(data = USvideos_NNA, in_set(seq(2006,2018,1), allow.na = FALSE), time_year)
assert(data = USvideos_NNA, in_set(seq(1,12,1), allow.na = FALSE), time_month)
assert(data = USvideos_NNA, in_set(seq(1,31,1), allow.na = FALSE), time_day)
assert(data = USvideos_NNA, in_set(seq(0,23,1), allow.na = FALSE), time_hour)
assert(data = USvideos_NNA, in_set(seq(0,59,1), allow.na = FALSE), time_minute)
assert(data = USvideos_NNA, in_set(seq(0,59,1), allow.na = FALSE), time_second)
assert(data = USvideos_NNA, in_set(c("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"), allow.na = FALSE), weekday)
#assert(data = USvideos_NNA, within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), chr_row_num)
assert(data = USvideos_NNA, within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), pattern_num)
assert(data = USvideos_NNA, within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), chr_length)
assert(data = USvideos_NNA, within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), chr_upper_num)
assert(data = USvideos_NNA, within_bounds(lower.bound = 0, upper.bound = Inf, allow.na = FALSE), chr_symbol_num)
```
